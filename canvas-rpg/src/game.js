...
(function(){
  ...
  class Game {
    constructor(canvas){
      ...
      this.player = {
        x: this.world.w/2, y: this.world.h/2, w: 18, h: 18,
    ...
    adjustCanvasSize() {
      ...
      if(bgImage && bgImage.complete && bgImage.naturalWidth > 0) {
        // 设置画布尺寸为背景图的原始尺寸
        const newWidth = Math.min(bgImage.naturalWidth, 1920); // 限制最大宽度
        const newHeight = Math.min(bgImage.naturalHeight, 1080); // 限制最大高度
...
目标：击败暗影魔王，拯救艾莉娅公主！`,
    ...
    spawnSlime(){
      ...
      const e = {
        ...
        hp: 10+zone, maxHp: 10+zone, // 测试：降低血量 10→3
        atk: 3+zone, def: 0+Math.floor(zone/2), spd: 60+zone*4,
        xp: 4+zone, gold: 2+Math.floor(zone/2), // 经验值保持不变
    ...
    spawnOrc(){
      ...
      const e = {
        ...
        hp: 50+zone*2, maxHp: 50+zone*2, // 测试：降低血量 35→8
        atk: 8+zone*2, def: 2+Math.floor(zone/2), spd: 45+zone*3,
        xp: 12+zone*2, gold: 5+Math.floor(zone*1.5), // 经验值保持不变
        hitCd: 0, touchCd: 0, dead:false,
      };
      this.enemies.push(e);
    }
    ...
    spawnFireBat(){
      ...
      const e = {
        ...
        hp: 40+zone, maxHp: 40+zone, // 测试：降低血量 15→4
        atk: 3+zone, def: 0, spd: 80+zone*2, // 调慢速度：180→80
        xp: 6+zone, gold: 5+Math.floor(zone/2), // 经验值保持不变
        hitCd: 0, touchCd: 0, dead:false,
      };
      this.enemies.push(e);
    }
    ...
    spawnLavaGiant(){
      ...
      const e = {
        ...
        hp: 100+zone*3, maxHp: 100+zone*3, // 测试：降低血量 80→15
        atk: 12+zone*3, def: 5+Math.floor(zone/2), spd: 60+zone*2, // 慢速但高攻击
        xp: 20+zone*3, gold: 25+Math.floor(zone*2), // 经验值保持不变
        hitCd: 0, touchCd: 0, dead:false,
      };
      this.enemies.push(e);
    }
    ...
    spawnIceArcher(){
      ...
      const e = {
        ...
        hp: 50+zone*2, maxHp: 50+zone*2, // 测试：降低血量 35→8
        atk: 8+zone*2, def: 1+Math.floor(zone/3), spd: 100+zone*3,
        xp: 15+zone*2, gold: 15+Math.floor(zone*1.5), // 经验值保持不变
        hitCd: 0, touchCd: 0, dead:false,
        // 远程攻击相关
        attackTimer: 0, attackCooldown: 2.0
      };
      this.enemies.push(e);
    }
    ...
    spawnIceKnight(){
      ...
      const e = {
        ...
        hp: 100+zone*4, maxHp: 100+zone*4, // 测试：降低血量 120→20
        atk: 15+zone*3, def: 4+Math.floor(zone/2), spd: 40+zone*4,
        xp: 25+zone*4, gold: 40+Math.floor(zone*3), // 经验值保持不变
        hitCd: 0, touchCd: 0, dead:false,
        // 冲锋攻击相关
        chargeTimer: 0, chargeCooldown: 4.0, charging: false
      };
      this.enemies.push(e);
    }
    ...
    spawnDemonLord(){
      ...
      const e = {
        ...
        hp: 500, maxHp: 500, // 测试：降低Boss血量 500→50
        atk: 20, def: 8, spd: 80,
        xp: 1000, gold: 500, // Boss经验值和金币保持不变
        hitCd: 0, touchCd: 0, dead:false,
        // Boss技能相关
        phase: 1, // 战斗阶段
        skillTimer: 0, skillCooldown: 3.0,
        shadowBalls: [], // 暗影球攻击
        summonTimer: 0
      };
      this.enemies.push(e);
    ...
    update(dt){
      ...
      for(const e of this.enemies){
        ...
        if(e.type === 'ice_archer') {
          e.attackTimer -= dt;
          if(dist <= 200 && e.attackTimer <= 0) { // 在200像素范围内攻击
            // 发射冰箭
            const arrow = {
              x: e.x + e.w/2 - 3,
              y: e.y + e.h/2 - 3,
...
感谢游玩！你是真正的英雄！`,
    ...
    drawEnvironment() {
      ...
      const camX = clamp(p.x + p.w/2 - canvas.width/2, 0, world.w - canvas.width);
      const camY = clamp(p.y + p.h/2 - canvas.height/2, 0, world.h - canvas.height);
      ...
      if(this.chapter === 1) {
        ...
        if(forestBg && forestBg.complete && forestBg.naturalWidth > 0) {
          // 背景图平铺或拉伸
          const bgScale = Math.max(canvas.width / forestBg.naturalWidth, canvas.height / forestBg.naturalHeight);
          const bgW = forestBg.naturalWidth * bgScale;
          const bgH = forestBg.naturalHeight * bgScale;
          const bgX = -((camX * 0.3) % bgW); // 视差滚动效果
          const bgY = -((camY * 0.3) % bgH);
          
          ctx.drawImage(forestBg, bgX, bgY, bgW, bgH);
          if(bgX > -bgW + canvas.width) ctx.drawImage(forestBg, bgX - bgW, bgY, bgW, bgH);
          if(bgY > -bgH + canvas.height) ctx.drawImage(forestBg, bgX, bgY - bgH, bgW, bgH);
        } else {
          // Fallback到代码绘制
        ...
        if(abyssBg && abyssBg.complete && abyssBg.naturalWidth > 0) {
          const bgScale = Math.max(canvas.width / abyssBg.naturalWidth, canvas.height / abyssBg.naturalHeight);
          const bgW = abyssBg.naturalWidth * bgScale;
          const bgH = abyssBg.naturalHeight * bgScale;
          const bgX = -((camX * 0.2) % bgW);
          const bgY = -((camY * 0.2) % bgH);
          
          ctx.drawImage(abyssBg, bgX, bgY, bgW, bgH);
          if(bgX > -bgW + canvas.width) ctx.drawImage(abyssBg, bgX - bgW, bgY, bgW, bgH);
          if(bgY > -bgH + canvas.height) ctx.drawImage(abyssBg, bgX, bgY - bgH, bgW, bgH);
        } else {
          // Fallback
        ...
        if(volcaBg && volcaBg.complete && volcaBg.naturalWidth > 0) {
          const bgScale = Math.max(canvas.width / volcaBg.naturalWidth, canvas.height / volcaBg.naturalHeight);
          const bgW = volcaBg.naturalWidth * bgScale;
          const bgH = volcaBg.naturalHeight * bgScale;
          const bgX = -((camX * 0.25) % bgW);
          const bgY = -((camY * 0.25) % bgH);
          
          ctx.drawImage(volcaBg, bgX, bgY, bgW, bgH);
          if(bgX > -bgW + canvas.width) ctx.drawImage(volcaBg, bgX - bgW, bgY, bgW, bgH);
          if(bgY > -bgH + canvas.height) ctx.drawImage(volcaBg, bgX, bgY - bgH, bgW, bgH);
        } else {
          // Fallback
        ...
        if(fortressBg && fortressBg.complete && fortressBg.naturalWidth > 0) {
          const bgScale = Math.max(canvas.width / fortressBg.naturalWidth, canvas.height / fortressBg.naturalHeight);
          const bgW = fortressBg.naturalWidth * bgScale;
          const bgH = fortressBg.naturalHeight * bgScale;
          const bgX = -((camX * 0.15) % bgW);
          const bgY = -((camY * 0.15) % bgH);
          
          ctx.drawImage(fortressBg, bgX, bgY, bgW, bgH);
          if(bgX > -bgW + canvas.width) ctx.drawImage(fortressBg, bgX - bgW, bgY, bgW, bgH);
          if(bgY > -bgH + canvas.height) ctx.drawImage(fortressBg, bgX, bgY - bgH, bgW, bgH);
        } else {
          // Fallback
        ...
        if(castleBg && castleBg.complete && castleBg.naturalWidth > 0) {
          const bgScale = Math.max(canvas.width / castleBg.naturalWidth, canvas.height / castleBg.naturalHeight);
          const bgW = castleBg.naturalWidth * bgScale;
          const bgH = castleBg.naturalHeight * bgScale;
          const bgX = -((camX * 0.1) % bgW);
          const bgY = -((camY * 0.1) % bgH);
          
          ctx.drawImage(castleBg, bgX, bgY, bgW, bgH);
          if(bgX > -bgW + canvas.width) ctx.drawImage(castleBg, bgX - bgW, bgY, bgW, bgH);
          if(bgY > -bgH + canvas.height) ctx.drawImage(castleBg, bgX, bgY - bgH, bgW, bgH);
        } else {
          // Fallback
    ...
    draw(){
      ...
      // 摄像机 - 基于背景图尺寸限制
      const bgBounds = this.getBackgroundBounds();
      const camX = clamp(p.x + p.w/2 - canvas.width/2, 0, Math.max(0, bgBounds.maxX - canvas.width));
      const camY = clamp(p.y + p.h/2 - canvas.height/2, 0, Math.max(0, bgBounds.maxY - canvas.height));
      ...
      if(this.chapter === 1) {
        ctx.fillText(`腐化史莱姆: ${this.killCount.slimes}/3`, 15, 50);
        ...
        ctx.fillText(`狂暴兽人战士: ${this.killCount.orcs}/2`, 15, 50);
        ...
        ctx.fillText(`火焰蝙蝠: ${this.killCount.fire_bats}/2 | 熔岩巨人: ${this.killCount.lava_giants}/1`, 15, 50);
        ...
        ctx.fillText(`冰霜射手: ${this.killCount.ice_archers}/2 | 寒冰骑士: ${this.killCount.ice_knights}/1`, 15, 50);
